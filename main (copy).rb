...
#How is this different from arrays?
  #arrays store data in a sequential, numerical index, so the only way to retrieve a specific item is to know this index number, which by itself doesn’t have much meaning.
...
# ** Keys can also be anything, but symbols (like :banana) & strings are the most common type of keys**
# ** Keys are unique, newer values overwrite older values**
# ** When you add the same key twice you change its value
# reference https://www.rubyguides.com/2020/05/ruby-hash-methods/

# Creating hashes

fruits = { coconut: 1, apple: 2, banana: 3 } # symbol hash
fruits[:orange] = 4 # add a new key-value pair to fruits
fruits[:orange] = 5 # change the value of the key :orange
fruits[:orange] # => 5  - access the value of the key :orange
# As an alternative, you can use the fetch method, which allows you to provide a default value.
fruits.fetch(:orange, 0) # => 5 - access the value of the key :orange, if value is not found, return 0
...
# example of hash array

  persons = [
    {
      first_name: 'John', # hash pair-value  key: value ***
      last_name: 'Doe',
      age: 30
    },
    {
  ...
  # get keys and values from a hash
